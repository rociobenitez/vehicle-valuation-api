document.addEventListener("DOMContentLoaded", () => {
  ...
  const GF_IDS = {
    form: "form-tasacion",
    inputData: "input_5_13",
    inputBase: "input_5_14",
    inputFinal: "input_5_29",
    inputEraNuevo: "input_5_21",
    inputExt: "input_5_22",
    inputLibro: "input_5_23",
    inputLlaves: "input_5_24",
    inputInt: "input_5_25",
    gformNextBtn: "gform_next_button_5_28",
    gformPrevBtn: "gform_previous_button_5_28",
    gformSubmitBtn: "gform_submit_button_5",
  };
  ...
  const loadToSelect = async (
    ...
    try {
      ...
      // si la API responde pero no hay datos o hay error,
      // mostrar mensaje en placeholder y deshabilitar select
      if (!data?.success) {
        renderSelectOptions(selectId, [], { placeholder: "No disponible" });
        select.disabled = true;
        if (typeof select._refreshNextState === "function")
          select._refreshNextState();
        return;
  ...
  getElement("next-4")?.addEventListener("click", async () => {
    ...
    resetSelect("version-select", "Selecciona una versión");
    await loadVersiones(makeId, modelId, year, doors); // <-- AQUÍ
    showStep(5);
  });
